...
class tqAlert{
    trigger(msg, opts){
        ...
        <div id="alertbox" class="center" style="width: 600px;">
        ...
        title.innerHTML = (opts.titleHTML || `<img class="responsive-img alert-img" src="/img/ltqrNEW.png" />`);
        alertbox.insertBefore(title, alertBoxInner);

        alertBoxBody.innerHTML = msg;

        let alertBoxFoot = document.querySelector('#alertboxfoot');
        alertBoxFoot.innerHTML = (opts.btnHTML || '<button id="btn-ok" class="btn cyan waves-light waves-effect" onclick="Alert.ok()">Ok</button>');
    ...
    ok(){
        // Para: user-idx => view
        (btnLink != null ? btnLink.innerHTML = `Copia el link aqu&iacute;
