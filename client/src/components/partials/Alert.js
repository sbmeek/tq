...
export default class Alert {
	...
	trigger = (msg, opts) => {
		...
		if ('undefined' !== typeof btnLink) btnLink.style.zIndex = '0'
		...
		alertTitle.innerHTML =
			opts.alertTitleHTML ||
			`<img class="responsive-img alert-img" src=${logo} />`
		...
		alertBoxFoot.innerHTML =
			opts.btnHTML ||
			`<button id="btn-ok" class="btn cyan waves-light waves-effect">Ok</button>`
	...
	recenter = () => {
		if (isAlerted) {
			...
			alertbox.style.left =
				winW / 2 - alertbox.getBoundingClientRect().width * 0.5 + 'px'
			...
			if (alertbox.getBoundingClientRect().width < 435) {
				...
				xBtn.setAttribute(
					'style',
					'font-size: 20px;min-width: 25px;min-height: 40px;'
				)
